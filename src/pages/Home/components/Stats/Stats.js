...
const Stats = (props) => {
  ...
  const getSuggestions = async (word) => {
    if (word) {
      setLoading(false);
      axios.get(`https://questkart.com/25offers/api/v1/user/search?query=${word}`)
    ...
    <div className="grid grid-cols-12 gap-2">
      ...
      <div className="col-span-6 w-full">
        <div class="shadow-md flex border border-gray-200 rounded-sm">
          <input class="w-full rounded-lg p-2 outline-none" type="text" onChange={(input) => updateValue(input.target.value)} placeholder="Search..." />
